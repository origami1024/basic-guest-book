<?php
	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
		  $cIndex = $row["id"];
		  echo  "<div class='container d-flex border p-2 flex-column m-1" . $animated . "'><div class='row'><div class='col-6 border-right'>" . $row["name"] . "</div><div class='col-6 text-right text-secondary'>" . $row["mail"]. "</div></div><div class='row py-1 '><div class='col-12 text-primary text-justify'>" . $row["text"] . "</div></div><div class='row mt-1'><div class='col-12 text-right text-secondary'>" . $row["date"] . "</div></div></div>";
			}
	};
	echo '","cIndex":'.$cIndex.'}';
	//echo 'represh v0';
